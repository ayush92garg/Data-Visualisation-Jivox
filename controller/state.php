<?php
	/*
		Controller to handle State Level Data queries
		Expects a valid state code and type
		Returs aggregated population data for the required state and input
	*/

	//inclduing head.php
	include "../includes/head.php";
	$state_code = $_GET["state_code"]; //code of state
	$type = $_GET["type"]; //type of data required
	...
	switch ($type) {//switch for the differnet values of type (Age/Type/Gender)
		case 'Age':
			$query = "select ageGroup,population from data where stateId = $state_code;";
			try{
			...
			break;
		case 'Gender':
			$query = "select numberOfMale, numberOfFemale from data  where stateId = $state_code;";
			try{
				$result = $dbh->query($query);
			}catch(Exception $e){
				write_error_to_file($e->getMessage(),"state_error_".date("Y-m-d H:i:s"));
			...
			break;
		case 'Type':
			$query = "select type, population from data  where stateId = $state_code";
			try{
				$result = $dbh->query($query);
			}catch(Exception $e){
				write_error_to_file($e->getMessage(),"state_error_".date("Y-m-d H:i:s"));
